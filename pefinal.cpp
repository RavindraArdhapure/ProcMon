...
bool ProcessInfo::ProcessLog()
{
	...
	FILE *fp;
	GetLocalTime(&lt);
	sprintf_s(FileName,"E:%02d_%02d_%02d %s.txt",lt.wHour,lt.wMinute,lt.wDay,month[lt.wMonth-1]);
...
bool ProcessInfo::ReadLog(DWORD hr,DWORD min,DWORD date,DWORD month) 
{
	char FileName[50];
	 ...
	 LOGFILE fobj;
	 FILE *fp;
	 sprintf_s(FileName,"E:%02d_%02d_%02d %s.txt",hr,min,date,montharr[month-1]);
	...
	while (1)
	{
		...
		if (count == 1)
		{
			if (_stricmp(command[0], "ps") == 0)
			{
				...
				ppobj->ProcessDisplay("-a");
				if (bret == FALSE)
				{
					cout << "ËRROR:Unable to display process" << endl;
					delete ppobj;
				}
			}
